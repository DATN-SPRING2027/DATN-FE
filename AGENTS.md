# AGENTS.md — Continuum AI Frontend

> Continuum AI frontend built from TailAdmin Free for Next.js.

## Project rules

- Follow the canonical decisions in the `DATN-SPRING2027/Document` repository.
- Use Next.js App Router, React, TypeScript, TailAdmin and Tailwind CSS.
- Reuse TailAdmin components before extending them; create a new primitive only when neither option fits.
- Keep server state in TanStack Query and local cross-page UI state in Zustand. Do not duplicate API data in Zustand.
- Treat endpoint examples as proposals. Do not invent request or response payloads before the shared OpenAPI contract is approved.
- The NestJS `backend-core` is a modular monolith and the FastAPI/SAG engine is a separate service. The frontend calls the approved backend/BFF contract, never SAG directly.
- Authentication tokens belong in HttpOnly cookies through the BFF/auth flow; never store access or refresh tokens in local storage.
- Enforce permissions on the backend. Frontend route and component guards improve UX but are not authorization controls.
- Keep secrets out of source control. Commit only `.env.example`, never a populated `.env` file.
- Every behavior change must pass `npm run check` before handoff.

## Imported foundation

- Edition: TailAdmin Next.js Free 2.4.0.
- Upstream: `TailAdmin/free-nextjs-admin-dashboard`.
- Pinned source commit: `4fba02489c93171220c13cd2b44cc0161ff6d2a1`.
- License: MIT; retain the root `LICENSE` file and attribution.
- Local changes: Continuum project metadata, planning documents, environment contract, and state-provider foundation.
